<template>
  <el-tabs v-model="activeTabName" type="card" class="layer-control">
    <el-tab-pane label="图层控制" name="图层控制">
      <el-collapse v-model="activeCollapseNames" class="layer-ctrl" accordion>
        <template>
          <el-collapse-item title="底图控制" style="text-align: center">
            <el-checkbox-group v-model="baseLayerCheckList">
              ...
              >
                <el-checkbox :label="layer" :disabled="layer.disabled">
...
export default {
  ...
  watch: {
    ...
    baseLayerCheckList(checkeds, olds) {
      ...
      if (newCheck) {
        const newCheckLayer = map.getLayerByProperty("name", newCheck.name);
      ...
      const unchecked = this.baseLayers.filter((i) => !checkeds.includes(i));
      checkeds.forEach((i) => {
