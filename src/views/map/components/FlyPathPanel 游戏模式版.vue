<template>
  <div class="toolbar" v-show="showFlyPathPanel">
    ...
    <div class="path-list">
      <el-dropdown @command="handlePathListCommand" style="margin-right: 10px;">
  ...
  methods: {
    handlePlayFlyPathClick() {
      if (!this.playingCurrentPath) {
        ...
        this.viewer.clock.onTick.addEventListener(this.onTickPlayPathListener);
      } else {
    ...
    add3DFlyPath(viewer) {
      ...
      Sandcastle.addToggleButton(
        ...
        checked => {
          if (checked) {
            ...
            exitRecordMode();
            pathPolyLine.polyline.positions = Object.values(cameraViews).map(
              i => i.destination
            );
          }
        },
      ...
      Sandcastle.addToolbarButton(
        ...
        () => {
          ...
          viewer.clock.onTick.addEventListener(onTickPlayPathListener);
          // const step = () => {
          //   const view = views.shift();
          //   if (view) {
          //     viewer.camera.setView(view);
      ...
      const clearFlyPath = () => {
        ...
        viewer.clock.onTick.removeEventListener(onTickPlayPathListener);
