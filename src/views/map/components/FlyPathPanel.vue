<template>
  <div class="flypath-toolbar toolbar" v-show="showFlyPathPanel && !asKanban">
    ...
    <el-button size="mini" type="primary" @click="hancleFlyPause"
      >暂停/继续</el-button
    >
    <el-button size="mini" type="primary" @click="handleClear">清除</el-button>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      currentPathId: "",
      currentPathName: "",
      speed: localStorage.getItem("flySpeed") || 1,
      height: localStorage.getItem("flyHeight") || 20,
      jiaodux: localStorage.getItem("flyJiaodux") || -50,
      jiaoduy: localStorage.getItem("flyJiaoduy") || 0,
    };
  },
  computed: {
    ...
    pathLength() {
      const length = this.coordinates.reduce((acc, cur, index, arr) => {
        if (index < arr.length - 1)
          acc += Cesium.Cartesian3.distance(cur, arr[index + 1]);
        return acc;
      }, 0);
  ...
  methods: {
    ...
    addFlyPointListener(movement) {
      ...
      if (cartesian) {
        ...
        cartesian = scene.globe.ellipsoid.cartographicToCartesian(cartographic);
    ...
    flyTickListener() {
      ...
      // 中心
      let center = this.entityFlyLine.position.getValue(
        viewer.clock.currentTime
      );
      // 当前位置的旋转矩阵
      let orientation = this.entityFlyLine.orientation.getValue(
        viewer.clock.currentTime
      );
      // 设置camera的坐标系
    ...
    hancleFlyPause() {
      this.entityFlyLine &&
        this.stopPoints
          .concat(this.entityFlyLine)
          .forEach(
            (i) => (i.show = !viewer.animation.viewModel.pauseViewModel.toggled)
          );
      viewer.animation.viewModel.pauseViewModel.command();
    },
